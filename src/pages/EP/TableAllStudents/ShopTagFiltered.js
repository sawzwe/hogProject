    ...
    <Stack flexGrow={1} direction="row" flexWrap="wrap" alignItems="center">
      ...
      {filterCategory !== 'All' && (
        <Panel label="Category:">
          <Chip size="small" label={filterCategory} onDelete={handleRemoveCategory} sx={{ m: 0.5 }} />
        </Panel>
      )}
      ...
      {(min !== 0 || max !== 200) && (
        <Panel label="Price:">
          <Chip size="small" label={`$${min} - ${max}`} onDelete={handleRemovePrice} sx={{ m: 0.5 }} />
        </Panel>
      )}

      {!!filterRating && (
        <Panel label="Rating:">
          <Chip size="small" label={sentenceCase(filterRating)} onDelete={handleRemoveRating} sx={{ m: 0.5 }} />
        </Panel>
      )}

      {isFiltered && (
        <Button color="error" size="small" onClick={onResetFilter} startIcon={<Iconify icon="eva:trash-2-outline" />}>
          Clear
        </Button>
      )}
    </Stack>
