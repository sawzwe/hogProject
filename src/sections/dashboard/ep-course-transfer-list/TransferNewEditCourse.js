...
//
import TransferCourseListDialog from './TransferCourseListDialog';

// ----------------------------------------------------------------------

export default function TransferNewEditCourse() {
    const {
        watch,
        setValue,
        formState: { errors },
        ...
        >
            <Stack sx={{ width: 1 }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                    ...
                    <TransferCourseListDialog
                        open={openFrom}
                        onClose={handleCloseFrom}
                        selected={(selectedId) => invoiceFrom?.id === selectedId}
                        onSelect={(address) => setValue('courseFrom', address)}
                        addressOptions={_invoiceAddressFrom}
                    />
                </Stack>

                <AddressInfo name={'Insert name'} address={'Insert address'} phone={'Insert phone'} />
            </Stack>
