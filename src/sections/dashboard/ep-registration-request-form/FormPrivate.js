...
FormPrivate.propTypes ={
    onAddStudent: PropTypes.func,
    onRemoveStudent: PropTypes.func,
}

export default function FormPrivate({ onAddStudent, onRemoveStudent }) {
    const {
        watch,
        setValue,
        formState: { errors },
        ...
        <Card sx={{ p: 3 }}>
            <Box rowGap={1}
                ...
                <Box gridArea={"studentText"} sx={{ pt: 1.2 }}>
                    <Typography variant="h6">{`Student(s)`} {assignedStudents?.length} / 1</Typography>
                </Box>
